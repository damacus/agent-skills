...
- **Do not track cookbook test policy locks**: Commit `Policyfile.rb`, add `Policyfile.lock.json`
  to `.gitignore`, and verify `git ls-files --error-unmatch Policyfile.lock.json` fails before
  opening a PR. Chef and Test Kitchen generate the lock during each test run. Never open a PR
  whose only purpose is to refresh `Policyfile.lock.json`.
...
`supports`, `Policyfile.rb` resolves cleanly (`chef install Policyfile.rb` exits 0),
`Policyfile.lock.json` is ignored and untracked, and any legacy `recipes/` or `attributes/`
directories are removed (if full migration scope).
...
- [ ] `Policyfile.lock.json` is ignored and `git ls-files Policyfile.lock.json` returns no file
